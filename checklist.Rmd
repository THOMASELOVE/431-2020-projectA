...
2.	The YAML in the R Markdown file produces what we want in the HTML
    ...
    - The date is properly formatted in the HTML (achieve this most easily by using **date: `r Sys.Date()`** in the YAML.)
    - There is a table of contents (doesn’t have to float, although we prefer this) which appears in the HTML and has section names that are appropriately and automatically numbered (achieve this through **number_sections: TRUE** in the YAML.)
...
4.	All section headings are correctly numbered and formatted, and describe effectively the contents of the material contained in that section.
    ...
    - Again, you need to be using `toc: TRUE` and `number_sections: TRUE` in your YAML.
